...
function initGraph(isEdit,scope) {
       ...
       // distanceWidget.set('distance', distance);
        ...
        displayInfo(distanceWidget,map,isEdit)
      }

      function displayInfo(widget,map,isEdit) {
    	  var scope;
    	  if($("#editPushCampaignWidget").length > 0){
        		  scope = angular.element($("#editPushCampaignWidget")).scope()
        		  scope.setGeoPushLocation(widget,map);
        	  }
    		 
       if($("#createSegmentedWidget").length > 0){ 
        	    scope = angular.element($("#createSegmentedWidget")).scope()
        	   scope.setGeoPushLocation(widget,map);
        	  }
      }
