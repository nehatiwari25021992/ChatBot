...
var fm = (function ($) {
    ...
    function appendFeedbackToBody(fm_options) {
        ...
        var titleStr = 'Help Improve AppHQ Console.';
        //       
        var appHQLoggedInUserStr = $("#appHQLoggedInUser").html();
        var appHQLoggedInUser  = appHQLoggedInUserStr.substring(0, appHQLoggedInUserStr.indexOf("@"));
        if (fm_options.show_form === true) {
            form_html = '<form class="feedback_me_form">'
            + '<div class="feedBack-custom-header">'+ titleStr + '</div>'
            +	'<ul>'
            +		'<li></li>'
            +		'<li></li>'
            +		'<li> <div>'+ 'Hello '+ appHQLoggedInUser + ',</div></li>'
            +         '<li><select class="form-control chosen-select feedback_type" id="feedbackForm" style="width:250px" data-placeholder="Feedback type..."><option value="UI Feedback">UI Feedback</option><option value="General Feedback">General Feedback</option></select></li>'
				
            +		 email_html

            +        '<li>    <div>' + fm_options.message_label + '</div> ' + message_asterisk + '<textarea rows="4" class="form-control ng-valid ng-dirty feedback_message" ' + message_required + ' placeholder="' + fm_options.message_placeholder + '" style></textarea> </li>'
            +		 radio_button_list_html
        ...
        $('body').append('<div onclick="fm.stopPropagation(event);fm.triggerAction(event,\'' + fm_options.position + '\');" class="feedback_trigger feedback_trigger_closed ' + fm_options.position + jQueryUIClasses1 + fm_class + jquery_class + bootstrap_class + "feedBack-custom" + '">'
    ...
    function sendFeedback(event, position) {
        ...
        if(window.location.href.indexOf("appWarp") > -1) {
            productType ="AppWarp";
        }
        else
        {
        ...
        dataArray = {
            ...
            feedbackType : $fm_content.find(".feedback_type").val(),
            ...
            success: function (data) {
                ...
                if (fm_options.delayed_close === true) {
                    if (fm_options.delayed_options.custom_html_success === undefined) {
                        ...
                        slideBack(fm_options, $fm_trigger, $fm_content);
                        $("body").append('<div style="background-color: #383c3f; color: #fff;" class="feedback-delayed-dlg success"><span class="feedback-sucess-message">' +
    ...
    function init(options) {
        ...
        var default_options = {
            ...
            submit_label : "Post your feedback",
            ...
            delayed_close : true,
            delayed_options : {
                delay_success_milliseconds : 1000,
                delay_fail_milliseconds : 2000,
                sending : "Sending...",
